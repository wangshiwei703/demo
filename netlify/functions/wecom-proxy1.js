...
exports.handler = async (event) => {
  ...
  try {
    ...
    // const recentUserIds = externalUserIds.slice(0, 20); // 限制到前20条
    // for (const externalUserId of recentUserIds) {
    //   // 获取联系人详情
    ...
    const batchResponse = await axios.post(
      ...
      {
        userid: body.userId,
        limit: 100 // 一次最多获取100个
      }
    );

    // 遍历批量返回的结果
    for (const contact of batchResponse.data.external_contact_list || []) {
